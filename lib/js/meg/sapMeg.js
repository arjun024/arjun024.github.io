...
Meg.prototype.makeTree = function(){
  ...
  var rootTreeNode = new sap.ui.commons.TreeNode({text:""+this.getContent(), expanded: true})
...
Meg.treeRenderer = function(root, rootTreeNode){
  ...
  for(var i in children){
    ...
    var childTreeNode = new sap.ui.commons.TreeNode({text:""+child.getContent(), expanded: true});
...
Meg.createMegTree = function(rootMeg){
  if(!rootMeg.getContent() || !rootMeg.getContent().getContent) return;
  if(rootMeg.getContent().getContent().length < 1) return;
  var rootContents = rootMeg.getContent().getContent();
...
Meg.dialog = function(sapRootObj){
  var githubT = new sap.ui.commons.TextView({}).addEventDelegate({
      onAfterRendering:function(){
        githubT.$().text("[Meg. Fork me on GitHub. ").append($("<a target='_blank'>github.com/arjun024/meg]</a>").attr("href","http://github.com/arjun024/meg"))
      }
    });
